...
static int ovl_copy_up_data(struct path *old, struct path *new, loff_t len)
{
	...
	//fzz_overlay: start
	//delay copy-up to read and write
	return 0;
	//fzz_overlay: end
...
int ovl_copy_up_block(struct file *lower_file, struct file *upper_file, loff_t pos, loff_t len)
{
	loff_t cloned;
	loff_t old_pos=pos;
	loff_t new_pos=pos;
	...
	if(len==0)
		return 0;
	cloned = do_clone_file_range(lower_file,pos,upper_file,pos,len,0);
	if(cloned == len)
		goto out;
	...
	while (len) {
		...
		bytes = do_splice_direct(lower_file, &old_pos,
					 upper_file, &new_pos,
					 this_len, SPLICE_F_MOVE);
		if (bytes <= 0) {
			error = bytes;
